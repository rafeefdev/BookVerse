...
void main() {
  ...
  group('LibraryNotifier - effectivePageCount', () {
    test(
      'finished when userPageCount < book.pageCount and currentPage >= userPageCount',
      () async {
        final book = Book(
          id: 'b1',
          title: 'Test Book',
    ...
    test(
      'currentlyReading when userPageCount > book.pageCount and currentPage < userPageCount',
      () async {
        final book = Book(
          id: 'b1',
          title: 'Test Book',
    ...
    test('finished when currentPage equals effectivePageCount', () async {
      ...
      when(() => mockRepo.getAllProgressWithBooks()).thenAnswer(
        (_) async => [
          ReadingProgressModel(bookId: 'b1', currentPage: 300, book: book),
        ],
      );
    ...
    test(
      'currentlyReading when userPageCount > 0 and currentPage is 0',
      () async {
        final book = Book(
          id: 'b1',
          title: 'Test Book',
    ...
    test('excludes progress with null book from both lists', () async {
      when(() => mockRepo.getAllProgressWithBooks()).thenAnswer(
        (_) async => [
          ReadingProgressModel(bookId: 'b1', currentPage: 100, book: null),
        ],
      );
