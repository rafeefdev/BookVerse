...
List<ReadingSessionModel> _randomSessions(int count, {int maxDays = 90}) {
  ...
  for (var i = 0; i < count; i++) {
    ...
    sessions.add(_randomSession(
      bookId: 'b${_rng.nextInt(5)}',
      day: day,
    ));
  }
  return sessions;
...
void main() {
  group('Fuzz: computeStreak invariants', () {
    ...
    test('never exceeds number of unique days', () {
      ...
      for (var i = 0; i < 100; i++) {
        ...
        final uniqueDays = sessions
            ...
            .length;
        final result = computeStreak(sessions, today);
        expect(result, lessThanOrEqualTo(uniqueDays),
            reason: 'streak $result > unique days $uniqueDays for ${sessions.length} sessions');
      }
    });
    ...
    test('only future sessions does not affect streak', () {
      ...
      for (var i = 0; i < 50; i++) {
        ...
        final result = computeStreak(sessions, today);
        expect(result, 0,
            reason: 'only future sessions should give streak 0');
      }
    });
  });
  ...
  group('Fuzz: computePagesInRange invariants', () {
    test('always returns >= 0 for any input', () {
      for (var i = 0; i < 100; i++) {
        ...
        final rangeStart = DateTime(2026, 6, 10).subtract(Duration(days: _rng.nextInt(90)));
        final range = all
    ...
    test('invariant: range pages <= sum of (endPage) when no startPage', () {
      for (var i = 0; i < 50; i++) {
        ...
        for (var j = 0; j < _rng.nextInt(20); j++) {
          sessions.add(ReadingSessionModel(
            bookId: 'b1',
            durationInSeconds: 0,
        ...
        expect(allTime, lessThanOrEqualTo(totalEndPage),
  ...
  group('Fuzz: cross-function consistency', () {
    test('computePagesInRange with all sessions matches computeAllTimePages', () {
      final today = DateTime(2026, 6, 10);
      for (var i = 0; i < 50; i++) {
        ...
        final rangeSessions = sessions
            ...
            .toList();
        final rangePages = computePagesInRange(rangeSessions, sessions, weekStart);
        final allPages = computeAllTimePages(sessions);
        expect(rangePages, lessThanOrEqualTo(allPages),
            reason: 'range pages $rangePages > all pages $allPages');
      }
    });

    test('computePagesInRange with single session equals its endPage when startPage=0', () {
      final today = DateTime(2026, 6, 10);
      for (var i = 0; i < 50; i++) {
        ...
        expect(result, endPage,
            reason: 'single session with startPage=0 should give endPage=$endPage, got $result');
      }
    });
    ...
    test('computePagesInRange with single session without startPage uses 0', () {
      ...
      for (var i = 0; i < 50; i++) {
        ...
        final result = computePagesInRange([session], [session], today);
        expect(result, endPage,
            reason: 'single session without startPage uses 0 fallback, should give endPage=$endPage, got $result');
      }
    });
  });
}
